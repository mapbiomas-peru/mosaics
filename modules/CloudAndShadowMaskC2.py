...
def cloudScoreMask(image, cloudThresh):
    ...
    temperature = image.select('tir')
    score = score.where(temperature.mask(), score.min(rescale(temperature, min=3000, max=2900)))
...
def cloudFlagMaskSr(image):
    ...
    #  Bit 2: Cirrus (high confidence),  Bit 3: Cloud, Bit 1: Dilated Cloud   
    #  in L5 Bit 2: Unused
    cloudMask = qaBand.bitwiseAnd(int(math.pow(2, 3))) \
                    .Or(qaBand.bitwiseAnd(int(math.pow(2, 1)))) \
                    .Or(qaBand.bitwiseAnd(int(math.pow(2, 2)))) \
                    .neq(0) \
                    .rename('cloudFlagMask')

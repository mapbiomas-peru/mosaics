...
def getNDFIb(image, endmembers):
    """Calculate ndfib and add them to image fractions
    ...
    Returns:
        ee.Image: Fractions image with shade, gvs and ndfib bands
    """

    outBandNames = ['gv', 'npv', 'soil', 'cloud','snow']
    ...
    # compute ndfib
    expression = '1.0 * ( ( gv / ( gv + npv + soil + snow ) ) - npv - soil - snow) / ( ( gv / (gv + npv + soil + snow ) ) + npv + soil + snow)'

    ndfib = fractions.expression(
        expression, {
            'gv': fractions.select('gv'),
            'npv': fractions.select('npv'),
            'soil': fractions.select('soil'),
            'snow': fractions.select('snow'),            
        }
    )

    # rescale NDFIb from 0 to 200 \
    ndfib = ndfib.multiply(100).add(100).byte().rename(['ndfib'])
    ...
    image = image.addBands(ndfib).addBands(snow)
